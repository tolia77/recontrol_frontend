...
function Sidebar({isOpen = false, onClose}: SidebarProps) {
    ...
    // Top content (logo + nav) - scrollable region
    const TopContent = (
        <>
            <Link to={"/dashboard"}>
            ...
            <nav className="space-y-[5px] text-white pl-[10px] mt-[30px]">
                ...
                {role === 'admin' && (
                    <Link to={"/admin/users"} className="flex gap-1 h-[45px] items-center">
                        <span
                            className="inline-flex h-5 w-5 items-center justify-center rounded bg-white/20 text-white">U</span>
                        <p>{t('nav.users')}</p>
                    </Link>
                )}
    ...
    // Language switch pinned to bottom
    const LanguageSwitch = (
        <div className="pl-[10px] mt-6 text-white">
            <label
                className="block text-xs uppercase tracking-wide opacity-70 mb-1">{t('lang.switch_label')}</label>
    ...
    return (
        <>
            {/* Desktop sidebar: flex column with scrollable top and pinned bottom */}
            ...
            {isOpen && (
                <div className="md:hidden fixed inset-0 z-50">
                    ...
                    <aside className="relative h-full w-[220px] bg-primary shadow-xl flex flex-col">
                        ...
                        <div className="mt-auto pb-6 px-[10px]">
                            {LanguageSwitch}
                        </div>
                    </aside>
                </div>
