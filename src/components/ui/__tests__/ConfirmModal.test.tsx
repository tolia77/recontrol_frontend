...
function defaultProps(overrides: Partial<React.ComponentProps<typeof ConfirmModal>> = {}) {
  return {
    open: true,
...
describe('ConfirmModal', () => {
  ...
  it('isBusy=true: a spinner is rendered inside the Confirm button', () => {
    render(
      <ConfirmModal {...defaultProps({ isBusy: true, confirmLabel: 'Delete' })} />,
    );
  ...
  it('dangerous=true: confirm button uses danger variant (bg-error)', () => {
    render(<ConfirmModal {...defaultProps({ dangerous: true, confirmLabel: 'Delete' })} />);
    const confirmBtn = screen.getByText('Delete').closest('button') as HTMLElement;
  ...
  it('dangerous=false: confirm button uses primary variant (bg-primary)', () => {
    ...
    const confirmBtn = screen.getByText('Save').closest('button') as HTMLElement;
    expect(confirmBtn.className).toContain('bg-primary');
  });

  it('calls onConfirm when confirm button is clicked', () => {
    const onConfirm = vi.fn();
    render(<ConfirmModal {...defaultProps({ onConfirm, confirmLabel: 'Yes' })} />);
