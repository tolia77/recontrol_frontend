export const NON_TEXT_THRESHOLD = 0.20;

export function normalizeClipboard(raw: string): { text: string; refused: boolean } {
  ...
  for (const ch of lf) {
    ...
    if (code <= 0x1F || code === 0x7F || (code >= 0x80 && code <= 0x9F)) control += 1;
  }
