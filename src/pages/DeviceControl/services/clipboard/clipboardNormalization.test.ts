...
import { normalizeClipboard, NON_TEXT_THRESHOLD } from './clipboardNormalization';

describe('normalizeClipboard', () => {
  it('strips embedded NUL bytes', () => {
    expect(normalizeClipboard('a\0b\0c')).toEqual({ text: 'abc', refused: false });
  });
  ...
  it('passes through plain ASCII text unchanged', () => {
    expect(normalizeClipboard('hello')).toEqual({ text: 'hello', refused: false });
  });
  ...
  it('exposes NON_TEXT_THRESHOLD as 0.20', () => {
    expect(NON_TEXT_THRESHOLD).toBe(0.20);
  });
});
