...
describe("prepareOutbound", () => {
  ...
  it("C7: hasFocus=false -> skip-not-focused", async () => {
    seqCounter = 0;
    const result = await prepareOutbound(
      baseInput("hello", { hasFocus: false }),
      nextSeq,
    );
...
describe("decideInbound", () => {
  ...
  it("C19: self-origin drop", async () => {
    ...
    env.originId = "OUR_ID";
    const result = await decideInbound(
      env,
      "OUR_ID",
      false,
...
describe("bindFocusVisibilityListeners", () => {
  ...
  it("L3: focus event calls onTrigger", () => {
    ...
    const captured: { focusHandler: (() => void) | null } = {
      focusHandler: null,
    };
    const fakeWindow = {
      addEventListener: vi.fn((type: string, fn: EventListener) => {
        if (type === "focus")
          captured.focusHandler = fn as unknown as () => void;
      }),
      removeEventListener: vi.fn(),
    };
    const fakeDoc = {
    ...
    const fakeDoc = {
      addEventListener: vi.fn((type: string, fn: EventListener) => {
        if (type === "visibilitychange")
          captured.visHandler = fn as unknown as () => void;
      }),
      removeEventListener: vi.fn(),
