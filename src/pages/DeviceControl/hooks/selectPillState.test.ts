...
function baseInput(
  overrides: Partial<SelectPillStateInput> = {},
): SelectPillStateInput {
  return {
    webRtcUp: true,
...
describe("selectPillState precedence ladder (D-02)", () => {
  ...
  it("rung 2a: unsupported-browser when both canRead and canWrite are false", () => {
    const r = selectPillState(
      baseInput({
        browserCaps: { canRead: false, canWrite: false, isSecureContext: true },
      }),
    );
  ...
  it("emits the correct tooltipKey for every state", () => {
    const cases: Array<{
      in: Partial<SelectPillStateInput>;
      expectedKey: string;
    }> = [
      ...
      {
        in: { hookStatus: "permission-required" },
        expectedKey: "pill.tooltip.permissionRequired",
      },
      {
        in: {
          cachedDesktopCaps: caps({
      ...
      {
        in: {
          lastRefusal: { reason: "TOO_LARGE", at: 9_500, source: "local" },
          now: 10_000,
        },
      ...
      {
        in: { lastSyncAt: 9_900, now: 10_000 },
        expectedKey: "pill.tooltip.pulsingJustSynced",
      },
