...
class MockWebSocket {
  ...
  dispatchBroadcast(inner: object, identifier: string = SCENARIO_RUN_IDENTIFIER): void {
...
function makeHarness() {
  ...
  const result = renderHook(({ socket }) => useScenarioRunChannel(socket, onBroadcast), {
...
describe('useScenarioRunChannel', () => {
  ...
  it('dispatches start_run as ActionCable message with scenario_id', () => {
    ...
    expect(frame.identifier).toBe(SCENARIO_RUN_IDENTIFIER);
    const inner = JSON.parse(frame.data) as { action: string; scenario_id: string };
