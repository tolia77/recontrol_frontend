...
export function normalizeWheelToClicks(
  deltaY: number,
  deltaMode: number,
): number {
  ...
  if (deltaMode === 1) {
    ...
    return lines === 0
      ? 0
      : Math.sign(lines) * Math.max(1, Math.round(Math.abs(lines)));
  }

  if (deltaMode === 2) {
    // DOM_DELTA_PAGE: approximate as multiple lines (heuristic)
    const approxLines = dy * 3;
    return approxLines === 0
      ? 0
      : Math.sign(approxLines) * Math.max(1, Math.round(Math.abs(approxLines)));
  }
  ...
  return clicks === 0
    ? 0
    : Math.sign(clicks) * Math.max(1, Math.round(Math.abs(clicks)));
}
