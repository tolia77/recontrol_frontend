    ...
    <button
      ...
      className={`flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-sm font-medium transition-colors ${
        active
          ...
          : 'text-darkgray hover:bg-gray-100 hover:text-text'
      }`}
    >
      {icon}
      <span>{label}</span>
      <span className="hidden text-[10px] font-normal text-darkgray/70 lg:inline">{hint}</span>
    </button>
  );
}
  ...
  return (
    <header className="flex h-[52px] shrink-0 items-center gap-3 border-b border-gray-200 bg-white px-3">
      ...
      <button
        ...
        className="rounded-lg p-1.5 text-darkgray transition-colors hover:bg-gray-100 hover:text-text"
      >
      ...
      >
        ...
        <span className="max-w-[12rem] truncate text-sm font-medium text-text">
      ...
      <div className="ml-auto flex items-center gap-2">
        {permissions?.see_screen && webRtcUp &&
        ...
        {permissions?.manage_power && <PowerPopover addAction={addAction} disabled={disabled} />}
        ...
        {permissions?.see_screen &&
          (isStreamActive ? (
            ...
            >
              ...
              <span>{isStreamBusy ? t('manual.quick.connecting') : t('manual.quick.stopStream')}</span>
            </button>
          ) : (
            <button
              type="button"
              ...
              className="rounded-lg bg-secondary px-3 py-1.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-primary disabled:cursor-not-allowed disabled:bg-lightgray disabled:text-darkgray"
            >
