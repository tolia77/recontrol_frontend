...
describe('copyAsMarkdown', () => {
  ...
  it('passes assistant markdown through unchanged', () => {
    const rows: Row[] = [
      { kind: 'assistant', id: 'a1', markdown: '**Hello** world', isStreaming: false },
    ];
    ...
    const rows: Row[] = [
      {
        ...
        startedAt: 0,
        endedAt: 100,
        result: { stderr: 'cat: /no/such/file: No such file or directory', exit: 1 },
      },
    ];
    const md = copyAsMarkdown(rows);
  ...
  it('preserves transcript order across mixed row kinds', () => {
    const rows: Row[] = [
      ...
      { kind: 'assistant', id: 'a1', markdown: 'Sure, running now.', isStreaming: false },
      {
        kind: 'tool',
        toolCallId: 't1',
        label: 'uptime',
