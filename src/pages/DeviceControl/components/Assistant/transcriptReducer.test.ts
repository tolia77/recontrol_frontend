...
describe('transcriptReducer', () => {
  it('submit_prompt appends operator row, resets stepCount, sets sessionToken, flips to streaming', () => {
    const next = transcriptReducer(initialTranscriptState, submit('hello', 'sess-1'));
  ...
  it('submit_prompt resets stepCount and quotaWarningShown when a new run begins', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'quota_warning', seq: 1, session_token: 's1', percent: 80 }),
    );
  ...
  it('starts a new assistant row when the previous one finished streaming (mid-loop assistant message)', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'done', seq: 1, session_token: 's', stop_reason: 'completed' }),
    );
  ...
  it('requires_confirmation + tool_call_start collapse into ONE tool row (D-05 / D-11)', () => {
    let s: TranscriptState = transcriptReducer(initialTranscriptState, submit('q', 's'));
    s = transcriptReducer(
      s,
      broadcast({
  ...
  it('quota_warning flips the once-per-run flag exactly once', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'quota_warning', seq: 0, session_token: 's', percent: 80 }),
    );
    expect(s.quotaWarningShown).toBe(true);
    const ref = s;
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'quota_warning', seq: 1, session_token: 's', percent: 80 }),
    );
    expect(s).toBe(ref);
  });
  ...
  it('first done wins; subsequent done is idempotent (RESEARCH §Pitfall 6)', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'done', seq: 0, session_token: 's', stop_reason: 'user_stopped' }),
    );
    expect(s.status).toBe('idle');
    const ref = s;
    s = transcriptReducer(
      s,
      broadcast({ type: 'done', seq: 1, session_token: 's', stop_reason: 'quota' }),
    );
    expect(s.status).toBe('idle');
  ...
  it('done(stop_reason: quota) transitions to halted_quota (D-07)', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'done', seq: 0, session_token: 's', stop_reason: 'quota' }),
    );
  ...
  it('error broadcast flips status to error and stops streaming assistant rows', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'error', seq: 1, session_token: 's', source: 'openrouter' }),
    );
    expect(s.status).toBe('error');
    const assistantRow = s.rows.find((r): r is AssistantMsgRow => r.kind === 'assistant');
  ...
  it('synthetic connection_lost error from the hook bypasses the session_token filter', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast({ type: 'error', seq: 0, session_token: '', source: 'connection_lost' }),
    );
    expect(s.status).toBe('error');
  });
  ...
  it('forward-compat: unknown broadcast types are ignored silently (STREAM-03)', () => {
    ...
    s = transcriptReducer(
      s,
      broadcast(
        { type: 'unknown_future_event', seq: 99, session_token: 's' } as unknown as AssistantBroadcast,
      ),
    );
    expect(s).toBe(ref);
  });
