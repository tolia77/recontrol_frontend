...
function StateIcon({ state, className }: { state: PillState; className?: string }) {
  switch (state) {
    case 'connected-idle':
    case 'pulsing':
      return (
        <span
          className={`inline-block w-2 h-2 rounded-full bg-white ${className ?? ''}`.trim()}
          aria-hidden="true"
        />
      );
    case 'paused':
  ...
  return (
    ...
    >
      <span className="flex items-center gap-2">
        <StateIcon state={result.state} className="w-4 h-4" />
