...
describe('DraftReviewModal', () => {
  ...
  const patternCases: Array<{ pattern: string; display: string; key: keyof typeof scenariosEn.ai.dry_intent }> = [
    { pattern: 'find_delete', display: 'find -delete', key: 'find_delete' },
    { pattern: 'dd_of_dev', display: 'dd of=/dev/', key: 'dd_of_dev' },
    { pattern: 'chmod_777_recursive', display: 'chmod -R 777', key: 'chmod_777_recursive' },
    { pattern: 'mkfs', display: 'mkfs', key: 'mkfs' },
    { pattern: 'truncate_zero', display: 'truncate -s 0', key: 'truncate_zero' },
    { pattern: 'redirect_to_system', display: '> /system path', key: 'redirect_to_system' },
    { pattern: 'rm_rf_root_adjacent', display: 'rm -rf', key: 'rm_rf_root_adjacent' },
  ];
  ...
  it('[Accept and save] click invokes onAccept exactly once', () => {
    ...
    render(
      <DraftReviewModal {...baseProps} draft={makeDraft()} onAccept={onAccept} />,
    );
  ...
  it('[Discard draft] click invokes onCancel exactly once', () => {
    ...
    render(
      <DraftReviewModal {...baseProps} draft={makeDraft()} onCancel={onCancel} />,
    );
  ...
  it('loading=true disables [Accept and save] and shows spinner state', () => {
    ...
    const accept = screen.getByTestId('draft-review-accept') as HTMLButtonElement;
