...
function defaultProps(overrides: Partial<React.ComponentProps<typeof MassDeleteConfirmModal>> = {}) {
  return {
    open: true,
    count: 5,
    onConfirm: () => {},
...
describe('MassDeleteConfirmModal', () => {
  it('renders nothing when open=false', () => {
    const { container } = render(<MassDeleteConfirmModal {...defaultProps({ open: false })} />);
  ...
  it('disables [Delete all] when typedPhrase is empty', () => {
    ...
    const confirm = screen.getByTestId('mass-delete-confirm') as HTMLButtonElement;
  ...
  it('disables both buttons when loading=true', () => {
    ...
    const cancel = screen.getByTestId('mass-delete-cancel') as HTMLButtonElement;
    const confirm = screen.getByTestId('mass-delete-confirm') as HTMLButtonElement;
