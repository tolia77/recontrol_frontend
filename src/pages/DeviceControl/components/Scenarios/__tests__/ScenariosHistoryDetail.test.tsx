...
import { initialTranscriptState, type ToolRow } from '../../Assistant/transcriptReducer';
...
function renderDetail(props: Partial<React.ComponentProps<typeof ScenariosHistoryDetail>> = {}) {
  return render(
    <ToastProvider>
...
describe('ScenariosHistoryDetail', () => {
  ...
  it('renders the header with scenario name, status badge, and run-id chip', async () => {
    ...
    expect(screen.getByTestId('history-detail-run-id-chip').textContent).toContain(
      'run-abcd',
    );
    // Status badge
  ...
  it('renders the live transcript when activeRun.runId matches runId', async () => {
    ...
    expect(screen.queryByTestId('history-detail-past-session-banner')).toBeNull();
  });
  ...
  it('renders the past-session banner when activeRun is null', async () => {
    ...
    expect(screen.getByTestId('history-detail-past-session-banner')).toBeDefined();
    expect(
  ...
  it('shows an error state when show() rejects', async () => {
    ...
    renderDetail();
    await waitFor(() => {
      expect(screen.getByTestId('scenarios-history-detail-error')).toBeDefined();
    });
  });
