...
describe('PolicyPreviewModal', () => {
  ...
  it('renders the error string when error is set', () => {
    ...
    expect(screen.getByTestId('policy-preview-error').textContent).toContain('boom');
  });
  ...
  it('formats the shell-like preview with quoted args containing whitespace', () => {
    ...
    expect(formatShellPreview('say', ['quote"inside'])).toBe('say quote"inside');
  ...
  it('renders the deny banner with one line per denied step, capped at 3 with overflow', () => {
    ...
    expect(within(banner).getByTestId('policy-preview-deny-overflow').textContent).toContain('2');
  });
  ...
  it('renders the drift banner when policy_drift=true', () => {
    const response = makeResponse([makeStep(0, 'allow')], { policy_drift: true });
  ...
  it('calls onApprove when [Run all] is clicked (no deny path)', () => {
    ...
    const commandSteps = [makeCommandStep('step-0', 'ls'), makeCommandStep('step-1', 'pwd')];
    render(
      <PolicyPreviewModal
        {...defaultProps}
        response={response}
