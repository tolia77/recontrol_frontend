...
export function PowerPopover({ addAction, disabled }: PowerPopoverProps) {
  ...
  const sendAction = (type: string) => {
    if (!addAction) {
      console.warn('No addAction provided to PowerPopover, cannot send command');
      return;
    }
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="rounded-lg p-2 text-darkgray transition-colors hover:bg-gray-100 hover:text-text disabled:cursor-not-allowed disabled:opacity-50"
      >
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 top-full z-50 mt-2 w-48 rounded-xl border border-gray-200 bg-white p-1.5 shadow-lg"
        >
          <h3 className="px-2 py-1 text-xs font-semibold uppercase tracking-wide text-darkgray">
          ...
          {POWER_OPTIONS.map((opt) => (
            <button
              ...
              className="w-full rounded-lg px-3 py-2 text-left text-sm text-text transition-colors hover:bg-gray-100"
            >
              {opt.label}
            </button>
          ))}
